...
We pass some funtion as parameter sometimes, such as \cd{.map(fn)}, \cd{.forEach(fn)}, \cd{.sort(compare)} \ldots 
...
Linear collection like \cd{LinkedList}, \cd{Stack}, \cd{Queue} \ldots have the default traversal order:
